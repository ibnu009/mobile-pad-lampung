...
import 'transaction_parkir_payment_confirmation_xendit_page.dart';

class TransactionXenditPaymentMethodPage extends StatefulWidget {
  final int quantity, price, idTransaksi;
  ...
  const TransactionXenditPaymentMethodPage(
      ...
      required this.price,
      required this.paymentType, required this.noTransaksi, this.noPol, required this.idTransaksi})
      : super(key: key);
...
class _TransactionXenditPaymentMethodPageState
  ...
  void calculateTotalPayment(XenditPaymentMethod paymentMethod) {
    if (paymentMethod.costType == "PERSENTASE") {
      totalPay = widget.price + (widget.price * paymentMethod.costValue) ~/ 100;
